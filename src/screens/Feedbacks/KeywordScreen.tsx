...
import {
    ...
    ActivityIndicator, Alert, useColorScheme, StatusBar, useWindowDimensions
} from 'react-native';
...
const KeywordScreen = () => {
    ...
    const fetchInitialData = async () => {
        ...
        try {
            if (isStudent) {
                // Students go straight to their subjects
                ...
                // Teacher/Admin sees assigned classes
    ...
    const handleSubjectSelect = async (sub: any) => {
        // Handle different structures from teacher vs student APIs
        ...
        try {
            ...
            setLessons(sortLessons(res.data)); // Apply sorting to lessons
            setViewStep('lessons');
    ...
    const handleSaveKeyword = async () => {
        ...
        try {
            ...
            handleLessonSelect(selectedLesson); // Refresh list
                    ...
                    {viewStep === 'keywords' && (
                        <>
                            {renderHeader(
                                ...
                                true, 
                                () => setViewStep('lessons'), 
                                true // Show Add Button if Teacher/Admin
                            )}
                            <ScrollView contentContainerStyle={{ padding: 15, paddingBottom: 50 }}>
                                {keywords.length > 0 ? keywords.map((kw) => (
                                    <View key={kw.id} style={[styles.keywordCard, { backgroundColor: COLORS.cardBg, borderColor: COLORS.border }]}>
                                        ...
                                        <View style={styles.keywordHeader}>
                                            <Text style={[styles.wordText, { color: COLORS.primary }]}>{kw.word}</Text>
                                            
                                            {!isStudent && (
                                                <View style={styles.actionButtons}>
                                                    <TouchableOpacity onPress={() => openKeywordForm(kw)} style={styles.actionIcon}>
                                                        <MaterialIcons name="edit" size={20} color={COLORS.textSub} />
                                                    </TouchableOpacity>
                                                    <TouchableOpacity onPress={() => handleDeleteKeyword(kw.id)} style={styles.actionIcon}>
                                ...
                                )) : <Text style={[styles.emptyText, { color: COLORS.textSub }]}>No keywords added for this lesson yet.</Text>}
                            </ScrollView>
                        </>
                    )}
...
const styles = StyleSheet.create({
    ...
    addBtnTop: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
    ...
    actionIcon: { marginLeft: 12, padding: 4 },
